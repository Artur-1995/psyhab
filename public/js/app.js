...
document.addEventListener("DOMContentLoaded", function () {
  ...
  $(searchEl).select2({
    ...
    allowClear: true,
    // Позволяет очищать поле
    tags: true,
    // Разрешить свободный ввод текста
    ajax: {
      ...
      dataType: 'json',
      delay: 250,
      // Задержка перед отправкой запроса (в мс)
  ...
  function formatItem(item) {
    ...
    return "\n                <a href=\"/blog/".concat(item.id, "-").concat(item.text, "\" target=\"_blank\" class=\"select2-result-link\">\n                    ").concat(item.text, "\n                </a>\n            ");
  }
