@extends('main')
@section('title', 'Блог | PSYHAB - психологическая помощь')
@section('content')
            ...
            <div data-slot="card" class="bg-card text-card-foreground flex flex-col gap-6 rounded-xl mb-8 border-0 shadow-lg">
                <div data-slot="card-content" class="[&amp;:last-child]:pb-6 p-6">
                    <div class="flex flex-col md:flex-row gap-4 items-center">
                        <div class="relative flex-1" style="max-width: 150px;">
                            <div class="flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" class="absolute inset-y-0 left-3 my-auto h-4 w-4 text-gray-400">
                        ...
                        {{-- <div class="flex flex-wrap gap-2">
                            <button
                                class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md font-medium transition-all disabled:pointer-events-none disabled:opacity-50 outline-none focus-visible:ring-2 focus-visible:ring-offset-2 bg-myCustomColor text-white hover:bg-myCustomColor-darkened h-8 px-3 text-sm">Все
                                статьи</button>
                            <button
                                class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md font-medium transition-all disabled:pointer-events-none disabled:opacity-50 outline-none focus-visible:ring-2 focus-visible:ring-offset-2 border border-gray-300 bg-white text-gray-900 hover:bg-gray-50 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-100 dark:hover:bg-gray-700 h-8 px-3 text-sm">Тревожность</button>
                            <button
                                class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md font-medium transition-all disabled:pointer-events-none disabled:opacity-50 outline-none focus-visible:ring-2 focus-visible:ring-offset-2 border border-gray-300 bg-white text-gray-900 hover:bg-gray-50 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-100 dark:hover:bg-gray-700 h-8 px-3 text-sm">Депрессия</button>
                            <button
                                class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md font-medium transition-all disabled:pointer-events-none disabled:opacity-50 outline-none focus-visible:ring-2 focus-visible:ring-offset-2 border border-gray-300 bg-white text-gray-900 hover:bg-gray-50 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-100 dark:hover:bg-gray-700 h-8 px-3 text-sm">Медитация</button>
                            <button
                        ...
                        @if ($topArticle)
                            <div class="relative">
                                ...
                                <div class="absolute top-4 left-4">
                                    <span
                                        class="inline-flex items-center justify-center rounded-md border px-2 py-0.5 text-xs font-medium whitespace-nowrap border-transparent text-white dark:bg-blue-700 bg-myCustomColor">Рекомендуемое</span>
                                </div>
                            </div>
                            <div data-slot="card-content" class="px-6 [&:last-child]:pb-6">
                                <div class="flex items-center gap-2 mb-3">
                                    ...
                                    <span class="text-sm text-gray-500">{{ $topArticle->created_at }}</span>
                                </div>
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-4 text-sm text-gray-500">
                                        <span>Автор:
                                            {{ $topArticle->user->first_name . ' ' . $topArticle->user->last_name }}</span>
                                ...
                                <a href="{{ route('article.show', ['slug' => Str::replace(' ', '_', preg_replace('/[^a-zA-Z0-9\s]/u', '', Str::transliterate($topArticle->title))), 'id' => $topArticle->id]) }}"
                                    class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md font-medium transition-all disabled:pointer-events-none disabled:opacity-50 outline-none focus-visible:ring-2 focus-visible:ring-offset-2 bg-myCustomColor text-white hover:bg-myCustomColor-darkened h-9 px-4 py-2 text-sm w-full mt-4">Читать
                                    полностью</a>
                            </div>
                        @endif
                        <div class="space-y-6">
                            @if ($articles)
                                <h2 class="text-2xl font-bold text-gray-900 pl-10">Последние статьи</h2>
                                @foreach ($articles as $article)
                                    <div data-slot="card"
                                        class="bg-card text-card-foreground flex flex-col gap-6 rounded-xl border-0 shadow-lg hover:shadow-xl transition-shadow">
                                        <div data-slot="card-content" class="[&:last-child]:pb-6 p-6">
                                            <div class="flex items-center gap-2 mb-3">
                                                <span
                                                    class="inline-flex items-center justify-center rounded-md border px-2 py-0.5 text-xs font-medium whitespace-nowrap border-gray-300 text-gray-900 dark:border-gray-600 dark:text-gray-100">{{ $article->category }}</span>
                                                <span class="text-sm text-gray-500">•</span>
                                                <span class="text-sm text-gray-500">{{ $article->created_at }}</span>
                                            </div>
                                            <h3
                                                class="text-xl font-semibold text-gray-900 mb-2 hover:text-blue-600 cursor-pointer">
                                                <a
                                                    href="{{ route('article.show', ['slug' => Str::replace(' ', '_', preg_replace('/[^a-zA-Z0-9\s]/u', '', Str::transliterate($article->title))), 'id' => $article->id]) }}">{{ $article->title }}
                                                </a>
                                            </h3>
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                @endforeach
                            @else
                                <p class="flex justify-center">Статьи отсутствуют</p>
                            @endif
                        </div>
                    </div>
                    <!-- Остальные статьи -->
                            ...
                            @foreach ($topArticles as $article)
                                <div class="flex gap-3">
                                    <div
                                        ...
                                        1</div>
                                    <div class="flex-1">
                                        <h4 class="font-medium text-sm hover:text-blue-600 cursor-pointer mb-1">
                                            <a
                                                href="{{ route('article.show', ['slug' => Str::replace(' ', '_', preg_replace('/[^a-zA-Z0-9\s]/u', '', Str::transliterate($article->title))), 'id' => $article->id]) }}">{{ $article->title }}
                                            </a>
                                        </h4>
